...
class Game {
    ...
    _transpose() {
        ...
        for(let col = 0; col < 4; col++){
            next[col] = [];
            for(let row = 0; row < 4; row++)next[col].push(this.state[row][col]);
        }
        this.state = next;
    }
    _statesEqual(a, b) {
    ...
    _addRandomTile() {
        const empty = [];
        for(let i = 0; i < 4; i++){
            for(let j = 0; j < 4; j++)if (this.state[i][j] === 0) empty.push([
                i,
                j
            ]);
        }
    ...
    _updateStatus() {
        ...
        if (!this._hasAvailableMoves()) this.status = "lose";
    }
    _hasAvailableMoves() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (c + 1 < 4 && this.state[r][c + 1] === val) return true;
            if (r + 1 < 4 && this.state[r + 1][c] === val) return true;
        }
        return false;
    }
}
